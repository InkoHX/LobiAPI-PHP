...
namespace LobiAPI\delion\HttpAPI;
...
class Http
{
    ...
    public function __construct(string $cookie_file_path = '')
    {
     ...
     * @return mixed
     */
    public function get(string $url, $header)
    {
        $ch = curl_init($url);
     ...
     * @return mixed
     */
    public function post(string $url, $data, $header)
    {
        $ch = curl_init($url);
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
